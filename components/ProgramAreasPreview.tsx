...
export default function ProgramAreasPreview() {
  return (
    <section id="programs" className="py-24 md:py-32" style={{ background: "var(--bg-2)" }}>
      <div className="wrap">
        <Reveal>
          <p className="eyebrow">
        ...
        <ul className="mt-14 grid md:grid-cols-2 gap-5">
          {programAreas.slice(0, 4).map((p, i) => (
            <Reveal key={p.slug} as="li" delay={i * 80}>
              <Link href={`/programs#${p.slug}`} className="luxury-card block h-full p-7 md:p-8 group">
                ...
                <h3 className="font-display text-3xl mt-6 leading-tight group-hover:text-[var(--gold-2)] transition-colors">
                  {p.name}
                </h3>
        ...
        <Reveal className="mt-12">
          <Link href="/programs" className="btn-outline inline-block px-7 py-3.5 text-[0.72rem] uppercase tracking-[0.16em]">
